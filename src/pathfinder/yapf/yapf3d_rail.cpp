                               ...
                               std::vector<std::pair<TileRef, Trackdir>> &out_neighbors)
{
	...
	if (cur_slice.kind != SliceKind::Track && cur_slice.kind != SliceKind::Portal) return;
	...
	if (neighbor_slice.kind != SliceKind::Track && neighbor_slice.kind != SliceKind::Portal) return;

	TileRef neighbor_ref{neighbor_tile, neighbor_sid};

	/* For Track slices, check which trackdirs are available. */
	DiagDirection enterdir = ReverseDiagDir(exitdir);

	if (neighbor_slice.kind == SliceKind::Track) {
		TrackBits tracks = static_cast<TrackBits>(neighbor_slice.track.tracks);
		TrackBits reachable = DiagdirReachesTracks(enterdir) & tracks;

		for (Track t = TRACK_BEGIN; t < TRACK_END; t++) {
			if (!(reachable & TrackToTrackBits(t))) continue;
			/* Generate trackdir for entering this track from enterdir. */
			Trackdir new_td = TrackEnterdirToTrackdir(t, enterdir);
		...
		Trackdir new_td = DiagDirToDiagTrackdir(enterdir);
		out_neighbors.emplace_back(neighbor_ref, new_td);
	}
}
...
Yapf3DResult Yapf3DFindPath(const Train *v, TileIndex entry_tile, Trackdir entry_td, TileIndex target_tile)
{
	...
	if (first_slice.kind != SliceKind::Track) return result;

	TrackBits reachable = DiagdirReachesTracks(enterdir) & static_cast<TrackBits>(first_slice.track.tracks);
	if (reachable == TRACK_BIT_NONE) return result;

	/* Create start nodes for each reachable trackdir. */
	for (Track t = TRACK_BEGIN; t < TRACK_END; t++) {
		if (!(reachable & TrackToTrackBits(t))) continue;
		Trackdir start_td = TrackEnterdirToTrackdir(t, enterdir);
	...
	while (!open.empty() && iterations < YAPF3D_MAX_NODES) {
		...
		/* Find node with lowest total cost (simple linear scan for MVP). */
		...
		/* Check if we reached an exit portal. */
		const TileSlice &cur_slice = _multilayer_map.GetSlice(current->ref.slice);
		if (cur_slice.kind == SliceKind::Portal) {
			/* Found exit portal! */
		...
		for (auto &[nref, ntd] : neighbors) {
			...
			int estimate = DistanceManhattan(nref.tile, target_tile) * YAPF3D_TILE_COST;
	...
	/* Build result from best portal node. */
	if (best_portal_node != nullptr) {
		result.path_found = true;
		result.exit_portal = best_portal_node->ref;
		result.total_cost = best_portal_node->cost;

		/* Walk back to find first trackdir. */
		Yapf3DNode *n = best_portal_node;
		while (n->parent != nullptr) {
			result.path.push_back(n->ref);
			if (n->parent->parent == nullptr) {
				result.first_trackdir = n->parent->td;
			}
			n = n->parent;
		}
		result.path.push_back(n->ref);
		if (n->parent == nullptr) {
			result.first_trackdir = n->td;
		}
	}
